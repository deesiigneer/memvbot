...
bot.delete_webhook()
WEBHOOK_HOST = "https://goldenrump.pythonanywhere.com/"
WEBHOOK_PATH = "Q2XSEsHHa92592g"
...
# Setup prices
prices = types.LabeledPrice(label='Телепортом', amount=10000)
...
async def command_start(message: types.Message):
    try:
        if message.from_user.language_code == 'ru':
            ...
            await bot.send_message(message.chat.id, 'Hello, i`m Meme Voice bot! I can send voice with memes in group, '
                                                    'channel or private message.\n'
                                                    '<a href="https://t.me/joinchat/AAAAAEsekoTUW0WjerW8wA">'
                                                    'News about bot you can find here</a> ', parse_mode='HTML')
    ...
    try:
        if message.from_user.language_code == 'ru':
            await bot.send_message(message.chat.id,
                                   ...
                                   f'\n_Разовое приобретение *1000* _просмотров - _*55₽*'
                                   f'\n'
        ...
        if message.from_user.id == deesiigneer:
            if message.voice.file_id != sql.one(
                ...
                if message.caption is not None:
                    ...
                    await bot.send_voice(chat_id=channel, voice=message.voice.file_id,
